...
class category extends admin {
	...
	public function init () {
		...
		if(!empty($result)) {
			foreach($result as $r) {
				...
				$r['help'] = '';
				$r['issys'] = $r['issys'] == 1 ? '<img src="statics/images/images/delete.png"/>' : '';
	...
	public function delete() {
		...
		if($categorys[$catid]['issys'] == 1){
			showmessage(L('删除失败，不能删除系统分类'),HTTP_REFERER);
			exit;
		}
		foreach($categorys as $key=>$value){
			if($value['parentid'] == $catid && $value['issys'] == 1){
				showmessage(L('删除失败，子分类中存在系统分类'),HTTP_REFERER);
				exit;
			}
		}
