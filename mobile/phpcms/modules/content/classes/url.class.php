		...
		if($upgrade || (isset($_POST['upgrade']) && defined('IN_ADMIN') && $_POST['upgrade'])) {
			...
			if($content_ishtml && $url) {
				...
				if($this->siteid > 1){
					...
					$url_arr[0] = $url_arr[1] = APP_PATH.$siteinfo['dirname'].'/'.$urls;
				}else{
	...
	public function category_url($catid, $page = 1) {
		...
		if (!$setting['ishtml']) { //如果不生成静态
			...
			if(strstr($urlrule,'{$catdir}')){
				if($page == 1){
					$url = str_replace(array('{$catdir}', '{$page}'), array($this->categorys[$catid]['catdir'], ''), $urlrule);
				}else{
					$url = str_replace(array('{$catdir}', '{$page}'), array($this->categorys[$catid]['catdir'], $page), $urlrule);
				}
			}else{
