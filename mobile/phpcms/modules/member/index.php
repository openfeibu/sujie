...
class index extends foreground {
	...
	function __construct() {
		...
		foreach($sitelist as $k=>$v){
			if($v['dirname'] == DIRNAME){
				$_REQUEST['siteid'] = $_GET['siteid'] = $v['siteid'];
				break;
			}
		}
	}
				...
				if($status > 0) {
					...
					if($userid > 0) {
						...
						if($userinfo['groupid'] == 7) {
							...
							$phpcms_auth = sys_auth($userid."\t".$userinfo['password'], 'ENCODE', md5(pc_base::load_config('system', 'auth_key').$this->http_user_agent));
					...
					if($member_setting['enablemailcheck']) {
						...
						$url = APP_PATH.DIRNAME.'/'."index.php?m=member&c=index&a=register&code=$code&verify=1";
	...
	public function login() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$phpcms_auth = sys_auth($userid."\t".$password, 'ENCODE', md5(pc_base::load_config('system', 'auth_key').$this->http_user_agent));
	...
	public function logout() {
		...
		if($setting['snda_enable'] && param::get_cookie('_from')=='snda') {
			...
			$forward = isset($_GET['forward']) && trim($_GET['forward']) ? urlencode($_GET['forward']) : '';
			$logouturl = 'https://cas.sdo.com/cas/logout?url='.urlencode(APP_PATH.DIRNAME.'/'.'index.php?m=member&c=index&a=logout&forward='.$forward);
	...
	public function public_sina_login() {
		...
		define('WEB_CALLBACK', APP_PATH.DIRNAME.'/'.'index.php?m=member&c=index&a=public_sina_login&callback=1');
	...
	public function public_snda_login() {
		...
		define('SNDA_CALLBACK', urlencode(APP_PATH.DIRNAME.'/'.'index.php?m=member&c=index&a=public_snda_login&callback=1'));
	...
	public function public_qq_login() {
		...
		if(isset($_GET['callback']) && trim($_GET['callback'])) {
			...
			$oauth_callback = APP_PATH.DIRNAME.'/'.'index.php?m=member&c=index&a=public_qq_login&callback=1';
	...
	public function public_forget_password () {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$url = APP_PATH.DIRNAME.'/'."index.php?m=member&c=index&a=public_forget_password&code=$code";
