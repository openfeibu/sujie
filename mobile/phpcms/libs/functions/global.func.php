...
function L($language = 'no_language',$pars = array(), $modules = '') {
	...
	$lang = defined('SYS_STYLE') ? SYS_STYLE : pc_base::load_config('system','lang');
...
function template($module = 'content', $template = 'index', $style = '') {
	...
	if(file_exists(ROOT_PC_PATH.'templates'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html')) {
		if(!file_exists($compiledtplfile) || (@filemtime(ROOT_PC_PATH.'templates'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html') > @filemtime($compiledtplfile))) {
		...
		if(!file_exists($compiledtplfile) || (file_exists(ROOT_PC_PATH.'templates'.DIRECTORY_SEPARATOR.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html') && filemtime(ROOT_PC_PATH.'templates'.DIRECTORY_SEPARATOR.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html') > filemtime($compiledtplfile))) {
			$template_cache->template_compile($module, $template, 'default');
		} elseif (!file_exists(ROOT_PC_PATH.'templates'.DIRECTORY_SEPARATOR.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html')) {
...
function pageurl($urlrule, $page, $array = array()) {
	...
	if(strpos($urlrule,APP_PATH) === FALSE){
		$url = APP_PATH.DIRNAME.'/'.$url;
	}
	$t = $_GET;
	unset($t['m']);
	unset($t['c']);
	...
	if(strpos($urlrule, '{$catdir}/{$page}') === false){
	    $url .= "&$urlvar1";
	}else{
	    $url .= "?$urlvar1";
	}
